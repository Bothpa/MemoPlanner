...
setInterval(() => {
  connection.query("SELECT 1", (err) => {
    if (err) {
      console.error("MySQL 연결 유지 실패: " + err.stack);
    } else {
      console.log("MySQL 연결 유지 성공");
    }
  });
}, 300000); // 300000ms = 5분
...
async function GetSchedule(year, month, userid, callback) {
  const selectQuery =
    "SELECT * FROM schedule WHERE year=? AND month=? AND userid=?  ORDER BY day ASC;";
...
async function InsertSchedule(
  ...
  const insertQuery =
    "INSERT INTO schedule (year, month, day, color, usertext, userid) VALUE (?,?,?,?,?,?) ;";
...
async function UpdateSchedule(
  ...
  const updateQuery =
    "UPDATE schedule SET year = ?, month = ?, day = ?, color = ?, usertext = ? WHERE id = ? AND userid = ?;";
...
async function GetMemo(userid, id, callback) {
  const getQuery =
    "SELECT id, userid, content, date FROM memo WHERE userid = ? AND id = ?;";
